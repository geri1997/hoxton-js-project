  ...
  startGameBtn.addEventListener("click", () => {
    getQuestions().then((questions) => {
      ...
      state.questions.forEach((question) => {
        question.correct_answer = parser.parseFromString(
...
function renderGame(mainEl) {
  ...
  questionDiv.setAttribute("class", "question");
  const questionH2 = document.createElement("h2");
  questionH2.innerHTML = `${state.question + 1}. ${
    state.questions[state.question].question
  }`;
  ...
  jokerSection.setAttribute("class", "joker-section");
  const jokerBtn = document.createElement("button");
  jokerBtn.setAttribute("class", "joker-btn");
  jokerBtn.textContent = `50/50 Joker`;
  jokerBtn.addEventListener("click", () => {
  ...
  const askGoogleBtn = document.createElement("button");
  askGoogleBtn.textContent = "Ask Google";
  askGoogleBtn.setAttribute("class", "joker-btn");
  askGoogleBtn.addEventListener("click", () => {
	state.time += 60;
  ...
  answersSection.setAttribute("class", "answers");
  renderAnswers(answersSection);
  jokerSection.append(jokerBtn, askGoogleBtn);
  mainEl.append(questionDiv, jokerSection, answersSection);
  if (state.questionAnswered) {
    if (!state.gameLost && state.question < 14) {
      const continueBtn = document.createElement("button");
      continueBtn.setAttribute("class", "game-button");
...
function renderAnswers(answersSection) {
  ...
  for (const answer of state.questions[state.question].shuffledAnswers) {
    ...
    answerBtn.addEventListener("click", () => {
      state.questionAnswered = true;
      if (
        answerBtn.textContent !== state.questions[state.question].correct_answer
      ) {
        state.gameLost = true;
...
function getQuestions() {
  return fetch(
    `https://opentdb.com/api.php?amount=15&category=${state.selectedCategory.id}&difficulty=${state.selectedDifficulty}&type=multiple`
  )
...
>>>>>>> d7f004382c186933be26255856c5a41a29761525
}
